...
## Tên
`openapi` (Tích hợp CLI `forgekit openapi_render`, `openapi_build_ui`)
...
- OpenAPI trong Forgekit hoàn toàn là một **Tài liệu (Docs artifact)**, không phải là công cụ sinh code (Codegenkit). Việc sinh mã nguồn Backend từ API Contract sẽ do các lệnh riêng biệt (như `forgekit api-gen`) của BE xử lý.
