...
## Chú ý quan trọng
- **Tuyệt đối cấm:** KHÔNG ĐƯỢC sinh ra file Markdown trực tiếp. Việc sinh file `.md` là nhiệm vụ của engine `/docskit` thông qua lệnh `split` và `render`. Kỹ năng này chỉ sinh `.bundle.yaml`.
