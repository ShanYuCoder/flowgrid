...
- **Sau đó:** Nếu được đánh dấu `approved`, có thể bàn giao cho hệ thống Backend Codegenkit (`/api` với cờ `--type=be`) để sinh code.
...
- Không bao giờ được đánh dấu "Ready for code" (sẵn sàng sinh code) nếu chưa có bằng chứng đã chạy vượt qua các lệnh kiểm định của `docskit`.
