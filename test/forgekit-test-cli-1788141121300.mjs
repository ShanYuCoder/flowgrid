...
import { intro, outro, select, multiselect, text, confirm as confirmPrompt, isCancel, cancel } from 'file:///tmp/forgekit-init-test-i1oFpC/mock-prompts.mjs';
...
async function main() {
  ...
  if (command === 'dev' || command === 'serve') {
    const { spawn } = await import('file:///tmp/forgekit-init-test-i1oFpC/mock-cp.mjs');
    const configPath = path.join(process.cwd(), '.forgekit', 'config.json');
  ...
  if (command === 'build') {
    const { spawnSync } = await import('file:///tmp/forgekit-init-test-i1oFpC/mock-cp.mjs');
    const configPath = path.join(process.cwd(), '.forgekit', 'config.json');
    let docsRoot, testsRoot;
    if (fs.existsSync(configPath)) {
      const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
