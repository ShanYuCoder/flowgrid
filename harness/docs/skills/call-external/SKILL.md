...
- **[MANDATORY]** Use `flowgrid_docs_route` or `flowgrid_docs_get_element` (or glob) to locate `…/api/<seq>/01-backend-spec.yaml` (or `common/yaml` / integrations `api/<seq>`). NEVER treat `ir/design.yaml` as the BE contract.
