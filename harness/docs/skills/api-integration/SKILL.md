...
- **[MANDATORY]** If an integration bundle already exists: run an audit before making modifications.
  - ✅ Check for existing endpoints under `surfaces/integrations/` using `docskit_route` or glob search before authoring a new contract trio.
...
- **[MANDATORY]** Every integration contract must be placed under:
  ...
  ├── 02-openapi.yaml         # generated by: docskit openapi:gen --spec <01>
  └── 03-mock-data.yaml
  ```
...
8. Run `docskit openapi:gen --spec …/api/<seq>/01-backend-spec.yaml` → generates `02-openapi.yaml`.
