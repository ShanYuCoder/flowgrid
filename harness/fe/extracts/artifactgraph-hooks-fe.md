...
- **Code size discipline:** Enforce `platform-code-size.mdc` (~200 lines/file, ~20 lines/func). Automatically split pages/composables/components during bộ code generation.
- Full product registry/IR remains in docs. bộ code reads it through the
  member-selected `FLOWGRID_DOCS_ROOT`; ArtifactGraph does not follow that
  pointer.
