...
import { CANONICAL_DIR, expectedCanonicalPath, indexIds, isRedirectStub, kindOf, relToRoot, walkMdFiles, SCAN_MD_DIRS, listFlowProcessFiles, } from '../scan/ids.js';
...
async function toolEngine(name, paths, projectRoot, extraArgs = []) {
    try {
...
function chapterHasId(docsRoot, id) {
...
export function registerTools(server) {
    server.tool('docskit_list_ids', 'List architecture/product IDs (CMP/FLOW/DEP/ADR/W/API/UI) from docs hub MD (skips redirect stubs)', {
        ...
        try {
            ...
            if (prefix)
                items = items.filter((i) => i.id.startsWith(prefix));
    ...
    server.tool('docskit_get_element', 'Get primary + related files and short excerpts for one ID (canonical path first)', {
        ...
        try {
            ...
            if (!meta)
                return text({ ok: false, error: 'not_found', id });
    ...
    server.tool('docskit_business_processes', 'List FLOW-* under architecture/03-business-process and surfaces/**/common/processes', {
        ...
        try {
            const root = resolveDocsRoot(docsRoot);
            const processes = listFlowProcessFiles(root);
            return text({
                ok: true,
    ...
    server.tool('docskit_docs_render', 'Render design Markdown from product YAML/bundles (no testcase MD)', {
        ...
        const extra = [];
        if (yamlRoot)
            extra.push('--yaml-root', yamlRoot);
        if (mdRoot)
            extra.push('--md-root', mdRoot);
        if (legacyRoot)
            extra.push('--legacy-root', legacyRoot);
        if (noIndex !== false)
            extra.push('--no-index');
