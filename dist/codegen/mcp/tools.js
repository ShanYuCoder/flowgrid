import { z } from 'zod';
import { resolveBeAdapter, resolveFeAdapter, resolveProjectRoot, } from '../config/project-root.js';
...
export function registerTools(server) {
    const register = (name, description, kind, script, dryRun = false) => {
    ...
    register('codegen_gen_dry', 'Dry-run FE portal codegen for the selected adapter.', 'codegen', 'generate.mjs', true);
    register('unit_gen', 'Run FE unit codegen for the selected adapter.', 'unitgen', 'generate.mjs');
    register('unit_gen_dry', 'Dry-run FE unit codegen for the selected adapter.', 'unitgen', 'generate.mjs', true);
    register('codegen_registry_validate', 'Validate FE design/codegen registries.', 'codegen', 'validate-registry.mjs');
    register('unit_registry_validate', 'Validate FE unit registries.', 'unitgen', 'validate-registry.mjs');
    ...
    server.tool('common_gen_dry', 'Dry-run shared UI inventory for surface or module common (no stub writes).', commonGenShape, async (input) => {
        try {
            const result = runCommonGen({
                adapter: resolveFeAdapter(input.adapter),
                projectRoot: resolveProjectRoot(input.projectRoot),
    ...
    const registerBe = (name, description, kind, dryRun = false) => {
    ...
    server.tool('common_registry_validate', 'Validate the target common registry and alias references.', {
        projectRoot: z.string().optional(),
        ...
        try {
            return text({
                ok: true,
                ...validateCommonRegistry(resolveProjectRoot(input.projectRoot), input.registry),
            });
        }
        catch (error) {
            return text({
