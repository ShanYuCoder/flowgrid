...
export function declaredRole(root) {
...
export function declaredProfileType(root) {
...
function validateFeAdapter(root, adapter, force) {
    if (adapter === 'dotnet-line') {
        if (!hasDotnetMarker(root) && !force) {
            throw new Error('Selected dotnet-line, but no .sln/.csproj found; pass --force only for a new base');
        }
        return;
    }
    const expected = adapter === 'nuxt4'
        ? ['nuxt.config.ts', 'nuxt.config.js']
        : ['next.config.ts', 'next.config.js', 'next.config.mjs'];
    if (!expected.some((file) => existsSync(path.join(root, file))) && !force) {
        throw new Error(`Selected ${adapter}, but no ${expected.join(' or ')} found; pass --force only for a new base`);
    }
}
function validateBeAdapter(root, adapter, force) {
    if (adapter === 'dotnet-integration') {
        if (!hasDotnetMarker(root) && !force) {
            throw new Error('Selected dotnet-integration, but no .sln/.csproj found; pass --force only for a new base');
        }
        return;
    }
    if (adapter === 'fastapi') {
    ...
    if (adapter === 'laravel') {
        ...
        const laravelRoot = roots.find((candidate) => existsSync(path.join(candidate, 'artisan')) &&
        ...
        if (laravelRoot && !force) {
            const composer = JSON.parse(readFileSync(path.join(laravelRoot, 'composer.json'), 'utf8'));
            const dependencies = {
                ...(composer.require ?? {}),
...
export function validateTarget(opts) {
    if (existsSync(path.join(opts.root, 'mcp-package.json')) && !opts.force) {
        throw new Error('Target looks like an MCP package (mcp-package.json present); Platform DNA installs only into docs/fe/be/monolith/tests hubs — not into MCP tooling repos');
    }
    const role = normalizeRole(declaredRole(opts.root));
    if (role === 'tooling' && !opts.force) {
        throw new Error('Repository declares role=tooling; Platform DNA does not install into MCP tooling repos');
    }
    if (role && role !== opts.type && !opts.force) {
        throw new Error(`Repository declares role=${role}, not ${opts.type}; use the correct --type or explicit --force`);
    }
    if (opts.type === 'monolith') {
        ...
        if (!opts.profile.feAdapters?.includes(feAdapter)) {
            throw new Error(`--fe-adapter for monolith must be ${opts.profile.feAdapters?.join(' | ')}`);
        }
        if (!opts.profile.beAdapters?.includes(beAdapter)) {
            throw new Error(`--be-adapter for monolith must be ${opts.profile.beAdapters?.join(' | ')}`);
        }
        ...
        if (!opts.adapter)
            throw new Error(`--adapter is required for --type=${opts.type}`);
        if (!opts.profile.adapters?.includes(opts.adapter)) {
            throw new Error(`--adapter for ${opts.type} must be ${opts.profile.adapters?.join(' | ')}`);
        }
        if (opts.type === 'fe')
            validateFeAdapter(opts.root, opts.adapter, opts.force);
        if (opts.type === 'be')
            validateBeAdapter(opts.root, opts.adapter, opts.force);
    }
    const markerFound = opts.profile.repoMarkers.length === 0 ||
        opts.profile.repoMarkers.some((marker) => markerPresent(opts.root, marker));
    if (!markerFound && !role && !opts.force) {
        throw new Error(`Repository does not look like a ${opts.type} base (expected one of: ${opts.profile.repoMarkers.join(', ')}); use --force only for a new empty base`);
    }
}
