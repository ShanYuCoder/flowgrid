...
const DEFAULT_BE_KEYWORD_HINTS = {
...
export function parseRegistryTagsLexicon(absPath) {
    ...
    for (const raw of text.split('\n')) {
        ...
        if (line.startsWith('#') && line.includes(':') && !line.startsWith('# ')) {
            ...
            if (line.startsWith('# Registry') || line.startsWith('# Suggested') || line.startsWith('# Alternatives'))
                continue;
        }
        ...
        for (const p of parts) {
            ...
            if (/(status\s*chip|badge|pill)/i.test(p)) {
            ...
            else if (/endpoint|route|controller|resource/i.test(p) &&
                ...
                let shadcnComp = '';
                if (p.includes('btn') || p.includes('button'))
                    shadcnComp = 'Button';
                else if (p.includes('dialog') || p.includes('modal'))
                    shadcnComp = 'Dialog';
                else if (p.includes('snackbar') || p.includes('toast'))
                    shadcnComp = 'Toast';
                else if (p.includes('text-field') || p.includes('input'))
                    shadcnComp = 'Input';
                else if (p.includes('data-table') || p.includes('table'))
                    shadcnComp = 'DataTable';
                else if (p.includes('checkbox'))
                    shadcnComp = 'Checkbox';
                else if (p.includes('radio'))
                    shadcnComp = 'RadioGroup';
                else if (p.includes('switch') || p.includes('slide-toggle'))
                    shadcnComp = 'Switch';
                else if (p.includes('select'))
                    shadcnComp = 'Select';
                else if (p.includes('chip') || p.includes('badge'))
                    shadcnComp = 'Badge';
                else if (p.includes('tabs') || p.includes('tab'))
                    shadcnComp = 'Tabs';
                else if (p.includes('expansion-panel'))
                    shadcnComp = 'Accordion';
                if (shadcnComp) {
...
export function parseTestTaxonomyLexicon(absPath) {
    const text = readFileSync(absPath, 'utf8');
    const types = [];
    const scenarios = [];
    const dimensions = { business: [], technical: [], quality: [] };
    const terms = [];
    let section = '';
    let dimKey = null;
    for (const raw of text.split('\n')) {
...
export function loadRegistryTagsLexicon(repoRoot, cfg) {
    const abs = resolveVocabularyPath(repoRoot, cfg, 'registryTags');
    if (!abs)
        return null;
    return parseRegistryTagsLexicon(abs);
}
export function loadTestTaxonomyLexicon(repoRoot, cfg) {
...
export function indexLexicons(store, repoRoot, cfg) {
    const summary = {};
...
function scoreTerm(haystack, term) {
    ...
    if (haystack.includes(t))
        return Math.min(1, 0.55 + t.length / 40);
    return 0;
}
/**
...
export function suggestTags(opts) {
    ...
    if (lane === 'fe' || lane === 'docs') {
        ...
        if (lex) {
            ...
            let profile = null;
            if (/(list|table|search|filter|danh sách|bảng)/i.test(bullets))
                profile = 'list';
            else if (/(create|form|new|tạo|form nhập)/i.test(bullets))
                profile = 'create';
            else if (/(detail|show|chi tiết)/i.test(bullets))
                profile = 'detail';
            if (profile)
                draftTags.push(...(DEFAULT_PROFILE_TAGS[profile] ?? []));
            for (const [term, tag] of Object.entries(lex.keywordHints)) {
                if (tag.startsWith('#needs-endpoint') || tag.startsWith('#needs-dto') || tag.startsWith('#api:'))
                    continue;
            ...
            for (const term of lex.terms) {
                if (term.startsWith('#api:') || term.startsWith('#needs-'))
                    continue;
                const s = scoreTerm(hay, term);
                if (s >= 0.7)
                    matches.push({ term, score: s });
            }
        }
    }
    if (lane === 'be') {
        ...
        if (lex)
            sourcePaths.push(lex.path);
        let beProfile = null;
        if (/(index|list|collection|danh sách)/i.test(bullets))
            beProfile = 'index';
        else if (/(store|create|post|tạo mới)/i.test(bullets))
            beProfile = 'store';
        else if (/(show|get by id|chi tiết|detail)/i.test(bullets))
            beProfile = 'show';
        else if (/(update|put|patch|cập nhật)/i.test(bullets))
            beProfile = 'update';
        else if (/(destroy|delete|xóa)/i.test(bullets))
            beProfile = 'destroy';
        if (beProfile)
            draftTags.push(...(DEFAULT_BE_PROFILE_TAGS[beProfile] ?? []));
        const hints = lex?.keywordHints ?? DEFAULT_BE_KEYWORD_HINTS;
        for (const [term, tag] of Object.entries(hints)) {
            const beTag = tag.startsWith('#needs-endpoint') ||
        ...
        if (lex) {
            for (const term of lex.terms) {
                if (!term.startsWith('#api:') && !term.startsWith('#needs-') && !term.startsWith('#data:'))
                    continue;
        ...
        for (const tag of needsFromDsl) {
            ...
            if (key && scoreTerm(hay, key.replace(/-/g, ' ')) > 0)
                draftTags.push(tag);
        }
    }
    if (lane === 'plans') {
        ...
        if (tax) {
            ...
            for (const dim of ['business', 'technical', 'quality']) {
...
//# sourceMappingURL=load-lexicon.js.map