 ...
 */
export function inspectAllowlistedCommand(repoRoot, cfg, commandKey, vars = {}) {
 ...
 */
export function materializeArgv(template, vars) {
 ...
 */
export function runAllowlistedCommand(repoRoot, cfg, commandKey, vars = {}) {
    const inspected = inspectAllowlistedCommand(repoRoot, cfg, commandKey, vars);
    if (!inspected.allowlisted || !inspected.argv) {
        throw new Error(`Command "${commandKey}" not allowlisted. Known: ${inspected.knownKeys.join(', ')}`);
    }
    const argv = inspected.argv;
    const bin = argv[0];
    if (!bin)
        throw new Error(`Empty argv for command "${commandKey}"`);
