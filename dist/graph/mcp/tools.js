...
import { loadEffectiveRepoConfig, loadRepoConfig, } from '../config/load-config.js';
...
import { inspectAllowlistedCommand, runAllowlistedCommand, } from '../gen/run-command.js';
import { detectStack } from '../config/platform-repos.js';
import { pathResolutionSummary, resolveGapSourceFiles, resolveSpecPath, } from '../config/resolve-paths.js';
...
export function registerTools(server) {
    ...
    server.tool('artifactgraph_projects', 'Show the current repository (legacy compatibility; product tools use cwd directly)', {}, async () => text({ projects: [currentProject()] }));
    /** Rebuild SQLite index from registries on disk (SSOT = product git; MCP = index only). */
    server.tool('artifactgraph_rebuild', 'Rebuild .artifactgraph/index.db from product registries/*.json (git remains SSOT; MCP never owns registry files)', {}, async () => {
     ...
     */
    server.tool('artifactgraph_analyze', 'Local preflight: analyze ir/spec.yaml and/or bullets → gaps, draft tags, cloudPromptSlice', {
        specPath: z
            .string()
            .optional()
        ...
        if (specPath) {
            ...
            result = analyzeBullets(project.root, cfg, bullets, store);
        else {
            store.close();
    ...
    server.tool('artifactgraph_grill_check', 'Grill helper: missing hashtags / needs-* / #missing_info candidates + A/B/C askUser prompts (do not invent business facts)', {
     ...
     */
    server.tool('artifactgraph_parity_check', 'Local parity: scan module legacy.fields and/or ingest parityFindings → parity-drift + askUser', {
        moduleDir: z
            .string()
            .optional()
    ...
    server.tool('artifactgraph_remember', 'Store grill or parity confirm in SQLite so later analyze can skip cloud / re-ask', {
        ...
        if (kind === 'parity')
            recordParityDecision(store, subject, choice, payload);
        else
    ...
    server.tool('artifactgraph_recommend_command', 'Inspect and materialize a product-owned allowlisted command without executing it. Run via FlowGrid (bộ docs / bộ code / bộ test).', {
        ...
        const project = currentProject();
        const cfg = loadEffectiveRepoConfig(project.root);
        return text(inspectAllowlistedCommand(project.root, cfg, commandKey, {
            spec: spec ?? '',
        }));
    });
    /** Token-light allowlist membership check; never executes. */
    server.tool('artifactgraph_allowlist_check', 'Check whether a command key is product-allowlisted and report its executable owner; never executes the command.', {
        commandKey: z.string(),
    }, async ({ commandKey }) => {
        const project = currentProject();
        const cfg = loadEffectiveRepoConfig(project.root);
    ...
    server.tool('artifactgraph_gaps', 'Same as analyze but returns only gaps[] + cloudPromptSlice (token-friendly)', {
        specPath: z.string().optional(),
        bullets: z.string().optional(),
    }, async ({ specPath, bullets }) => {
        const project = currentProject();
     ...
     */
    server.tool('artifactgraph_api_reuse_check', 'Check if an API path/entity action already exists in product surfaces before creating a new spec. Returns matches[] and draftTags with #reuse-api when found.', {
     ...
     */
    server.tool('artifactgraph_suggest_tags', 'Suggest draftTags / taxonomy enums from project/package lexicons (lane=fe|docs|be|plans).', {
    ...
    server.tool('artifactgraph_status', 'Show project root, stack, dsl lanes, command keys; registries SSOT = product repo paths', {}, async () => {
        const project = currentProject();
