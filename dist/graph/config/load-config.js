...
export function parseRepoConfig(input) {
    return artifactgraphConfigSchema.parse(input);
}
/** Standalone defaults when a repo has not been initialized yet. */
export function defaultRepoConfig(projectId = path.basename(process.cwd())) {
    return {
        version: 2,
        stack: 'generic',
        mode: 'brownfield',
...
export function loadEffectiveRepoConfig(repoRoot) {
...
export function requireRepoConfig(repoRoot) {
    const cfg = loadRepoConfig(repoRoot);
    if (!cfg) {
        throw new Error(`Missing ${CONFIG_NAME} in ${repoRoot}. Run: artifactgraph init`);
    }
    return cfg;
}
/** Load stack preset from this package (stacks/nuxt4.json, …). */
export function loadStackPreset(stack) {
 ...
 */
export function writeBrownfieldConfig(repoRoot, opts) {
