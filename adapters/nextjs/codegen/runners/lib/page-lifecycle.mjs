...
export function routePathFromPageFile(relativePagePath) {
  ...
  if (normalized === 'src/app/not-found.tsx') return '/404'
  if (normalized === 'src/app/error.tsx') return '/error'

  const grouped = normalized.match(/^src\/app\/\((?:dashboard|auth|public)\)\/(?:(.+)\/)?page\.tsx$/)
  if (grouped) return grouped[1] ? `/${grouped[1]}` : '/'
...
export async function syncPageLifecycleFromManifests(root) {
  ...
  for (const [routePath, entry] of Object.entries(data.routes)) {
    ...
    const specFileAbs = entry.spec ? path.join(root, entry.spec) : null
    const specDir = specFileAbs ? path.dirname(specFileAbs) : null
    const featureDir = specDir ? (path.basename(specDir) === 'ir' ? path.dirname(specDir) : specDir) : null
    const manifestPath = featureDir ? path.join(featureDir, 'generated', 'codegen.manifest.json') : null
