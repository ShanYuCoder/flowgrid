    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      if (entry.isDirectory() && !entry.name.startsWith('.')) {
        ...
        if (entry.name.startsWith('CMP-')) {
          const mod = { id: entry.name, slug: entry.name.toLowerCase(), codeIds: [] }
          modules.push(mod)
  ...
  if (/^[A-Z]+-[A-Z]+(?:-\d{3})?(?:-[A-Z0-9]+)?$/i.test(id)) {
    ...
    for (const mod of matchedModules) {
      for (const cid of mod.codeIds || []) {
        ...
        try {
          const sub = resolveHubId(repoRoot, cid, 'codegen')
          paths.push(...sub.paths)
          notes.push(...sub.notes)
        } catch (e) {
    ...
    if (!paths.length) {
      throw new Error(`Cluster/Module ${id}: no ir/design.yaml after /grill-dev`)
    }
    return { kind: 'module-code', id, paths, notes }
  }
