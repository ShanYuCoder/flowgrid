...
function findFlowMarkdown(docsRoot, id) {
  ...
  const walk = (dir) => {
    ...
    for (const ent of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (ent.name === `${id}.md` && p.split(path.sep).join('/').includes('/common/processes/')) {
        hits.push(p)
      }
    }
  }
...
export function resolveHubId(repoRoot, id, mode = 'testcase') {
  ...
  // FLOW-* before tests-hub lookup (no platform-repos required)
  if (/^FLOW-/i.test(id)) {
    if (mode === 'codegen') {
      throw new Error(`FLOW is a process id, use SC-* or W-* (not ir/design.yaml)`)
    }
    const docsRoot = getDocsRoot()
    const md = findFlowMarkdown(docsRoot, id)
    if (!md) {
      throw new Error(`Unknown FLOW ${id} — expected architecture/03-business-process/${id}.md or …/common/processes/${id}.md`)
    }
  ...
  if (/^(W|API|UI)-/i.test(id) || /^[a-z]+-[a-z]+-\d{3}(-\d{2,3})+$/i.test(id)) {
    ...
    if (!spec) throw new Error(`No ir/design.yaml under ${rel}`)
  ...
  if (/^[A-Z]+-[A-Z]+-\d{3}$/i.test(id)) {
    ...
    if (!paths.length) {
      throw new Error(`CMP ${id}: no gen-ready W-* specs (need ir/design.yaml after /grill-dev + spec:split)`)
    }
  ...
  if (/^DEP-/i.test(id)) {
    throw new Error(`DEP-* is architecture, not a codegen/testcase file id`)
  }
